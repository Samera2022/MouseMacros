...
| `available_version` | string | 从发布源获得的最新可用版本。 |

### 可选字段

| 字段 | 类型 | 描述 |
|------|------|------|
| `soft_dependencies` | string[] | 脚本名称数组，这些脚本增强功能但不是必需的。缺少软依赖不会阻止脚本运行。 |
| `hard_dependencies` | string[] | 脚本名称数组，这些脚本是必需的。如果缺少任何硬依赖，脚本将无法正常工作。 |
| `requires_native_access` | boolean | 如果脚本需要原生系统访问权限，设置为 `true`。需要显式的用户批准。 |
| `native_access_description` | string | 说明为什么需要原生访问权限。在用户启用脚本时显示给用户。 |
